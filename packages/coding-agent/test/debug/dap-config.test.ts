	...
	it("auto-selects Bun only under Bun root markers", async () => {
		...
		try {
			await withBunDapCommand(cwd, async () => {
				...
				const withoutRoot = selectLaunchAdapter(program, cwd);
				expect(withoutRoot.kind === "adapter" ? withoutRoot.adapter.name : withoutRoot.kind).not.toBe("bun");

				await Bun.write(path.join(cwd, "bun.lock"), "");
				expect(requireSelectedAdapter(selectLaunchAdapter(program, cwd)).name).toBe("bun");
			});
		} finally {
			await fs.rm(cwd, { recursive: true, force: true });
		}
